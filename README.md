...
После рабочего созвона читает транскрипт, записанный `live_copilot_poc`, и
двумя независимыми проходами LLM: (1) находит устно поручённые задачи и
заводит тикеты в Jira, (2) собирает Q&A-саммари реально обсуждённых
вопросов и решений — см. раздел «Саммари прошлых созвонов» ниже.
Дизайн: `docs/superpowers/specs/2026-08-27-jira-task-detection-mvp-design.md`
(Jira) и `docs/superpowers/specs/2026-08-29-meeting-summary-qa-design.md`
(саммари).
...
- `~/.credentials/openrouter_api_key.env` — уже должен существовать, если
  `daily_standup_matcher` настроен (регистрация на openrouter.ai бесплатная,
  без карты). Формат: `OPENROUTER_API_KEY=...`
...
Помимо задач для Jira, каждый прогон `run.py` (кроме случаев, когда
транскрипт короче `MIN_TRANSCRIPT_CHARS` и пропускается целиком) дополнительно
сохраняет структурированное саммари звонка — не общий пересказ, а список
реально обсуждённых вопросов/тем и кратких ответов/решений по каждому — в
`summaries/<имя транскрипта без расширения>.md`. Эта папка не в git (личный
рабочий контент, не пример для репозитория).

Сразу под заголовком — строка **«Темы»**: список вопросов из тех же
Q&A-пар, без отдельного LLM-вызова (по образцу Granola/Fireflies — общая
тема звонка отдельно от подробного разбора).

Генерация саммари — независимый шаг: если он упадёт (сеть, лимит
OpenRouter, сбой при сохранении файла), это никак не повлияет на уже
созданные тикеты в Jira и не помешает переносу транскрипта в
`processed/` — просто в Telegram-отчёте будет пометка «не удалось
сгенерировать саммари» или «не удалось сохранить саммари».
...
**Как этим пользоваться:** больше не нужно вручную перетаскивать файлы —
`live_copilot_poc` сам читает `summaries/` напрямую с диска: панель
«Прошлые созвоны» в окне суфлёра сама показывает самое свежее саммари при
открытии (см. README `live_copilot_poc`, PR #1).
...
Обязательно нужен свой `~/.credentials/openrouter_api_key.env` — без него
скрипт падает сразу на детекте задачи, до всего остального.
...
Все внешние вызовы (OpenRouter, Jira, Telegram) в тестах замоканы — `pytest`
...
9. Проверить, что появился `summaries/<имя транскрипта>.md` и в нём есть
   секция «## Обсуждали» с реально содержательными вопросами по этому
   созвону — не просто наличие файла, а что вопросы и ответы в нём
   действительно соответствуют тому, что обсуждалось (промпт и схема
   саммари нигде не проверяются на реальной модели — только моками в
   `pytest`, так что здесь единственное место, где эта проверка вообще
   происходит).
10. Перетащить этот файл из `summaries/` в «Базу знаний» `live_copilot_poc`
    и спросить его текстом про этот созвон — убедиться, что файл реально
    пригоден для загрузки и что ответ по содержанию совпадает с тем, что
    было в разговоре.
