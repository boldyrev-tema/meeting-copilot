...
поручением задачи (Артёму) и посторонней светской репликой, которую скрипт
должен проигнорировать. Имя в файле («Артём») уже есть в закоммиченном
`name_mapping.json` — для осмысленного теста с реальным Jira-исполнителем
поправь `name_mapping.json` под своё имя/логин перед запуском.
