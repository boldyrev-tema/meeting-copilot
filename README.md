...
Генерация саммари — независимый шаг: если он упадёт (сеть, лимит Groq, сбой
при сохранении файла), это никак не повлияет на уже созданные тикеты в Jira
и не помешает переносу транскрипта в `processed/` — просто в Telegram-отчёте
будет пометка «не удалось сгенерировать саммари» или «не удалось сохранить
саммари».
